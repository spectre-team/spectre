...
namespace Spectre.Algorithms.Results
{
    ...
    public class GmmModel
    {
        ...
        private void ParseMatlabStruct(object matlabModel)
        {
            ...
            OriginalMeanSpectrum = flatten(arg: (double[,])model.GetField(fieldName: "meanspec"));
            PeakLocations = flatten(arg: (double[,])model.GetField(fieldName: "mu"));
            PeakWidths = flatten(arg: (double[,])model.GetField(fieldName: "sig"));
            PeakHeightMultipliers = flatten(arg: (double[,])model.GetField(fieldName: "w"));
        }
